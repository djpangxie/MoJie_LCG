...
class Player(Player_Group):
    def __init__(self, main_game):
        ...
        self.card_file = "伊姆拉崔的学识.jpg"  # 卡牌的图像文件名
        self.card_image = pygame.image.load(os.path.join(self.card_dir, self.card_file)).convert()  # 卡牌的原始图像
        self.card_name = "伊姆拉崔的学识"  # 卡牌的名称
        ...
        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "行动：选择一名角色。治疗该角色的所有伤害。"

        # 剧情描述的斜体文字
        self.describe_text = "...他们要的是理解、创造和医疗...这些事物是中土世界的精灵牺牲许多才换来的。 ——爱隆，《魔戒现身》"
        ...
        if self.card_order[0] == "打出事件后" and self.card_order[1] == 0:
            if self.card_order[2]:
                ...
                if card_group:
                    ...
                    self.main_game.information = [0, "事件卡牌将要治疗", self, self.select_card]
                else:
                    self.card_order[2] = 1
                self.card_order[1] += 1
            ...
            if self.card_order[2]:
                ...
                self.main_game.information = [0, "事件卡牌治疗后", self, self.select_card]
                self.card_order[1] += 1
