...
class Player(Player_Group):
    def __init__(self, main_game):
        ...
        self.card_file = "密径.jpg"  # 卡牌的图像文件名
        self.card_image = pygame.image.load(os.path.join(self.card_dir, self.card_file)).convert()  # 卡牌的原始图像
        self.card_name = "密径"  # 卡牌的名称
        ...
        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "任务行动：选择一个在场景区的地区。该地区不计算它的威胁值直到本阶段结束。"

        # 剧情描述的斜体文字
        self.describe_text = "...比翁建议他们可以走这条路，因为从这边往北骑几天，就会来到幽暗密林中一条鲜为人知的道路入口... ——《哈比人》"
            ...
            if self.card_order[2]:
                ...
                for card in self.main_game.scenario_area.card_group:
                    if card.card_type == "地区" and "免疫" not in card.rule_mark:
                        regions.append(card)
                if regions:
                    self.active_condition["目标卡牌"] = [self.main_game.card_select(regions)]
