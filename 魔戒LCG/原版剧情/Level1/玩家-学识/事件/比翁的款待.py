...
class Player(Player_Group):
    def __init__(self, main_game):
        ...
        self.card_file = "比翁的款待.jpg"  # 卡牌的图像文件名
        self.card_image = pygame.image.load(os.path.join(self.card_dir, self.card_file)).convert()  # 卡牌的原始图像
        self.card_name = "比翁的款待"  # 卡牌的名称
        ...
        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "行动：选择一位玩家。治疗该玩家控制的每名英雄的所有伤害。"

        # 剧情描述的斜体文字
        self.describe_text = "...只要你们还有机会回来，我的大门随时为诸位敞开。 ——比翁，《哈比人》"
        ...
        if self.card_order[0] == "打出事件后" and self.card_order[1] == 0:
            ...
            if self.card_order[2]:
                ...
                if self.card_order[5]:
                    if self.hero_group:
                        self.hero_group.pop(0)
                    self.card_order[4] += 1
                    self.card_order[5] -= 1
                elif self.card_order[4] % 2:
                    if self.hero_group:
                        self.hero_group[0].active_health_point = self.hero_group[0].health_point
                        self.hero_group[0].update_mask()
                        self.main_game.information = [0, "事件卡牌治疗后", self, self.hero_group.pop(0)]
                    self.card_order[4] += 1
                else:
                    if self.hero_group:
                        self.main_game.information = [0, "事件卡牌将要治疗", self, self.hero_group[0]]
                        self.card_order[4] += 1
                    else:
