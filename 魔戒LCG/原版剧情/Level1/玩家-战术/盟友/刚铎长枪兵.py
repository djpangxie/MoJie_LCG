...
class Player(Player_Group):
    def __init__(self, main_game):
        ...
        self.card_file = "刚铎长枪兵.jpg"  # 卡牌的图像文件名
        self.card_image = pygame.image.load(os.path.join(self.card_dir, self.card_file)).convert()  # 卡牌的原始图像
        self.card_name = "刚铎长枪兵"  # 卡牌的名称
        ...
        self.card_attribute = ("刚铎", "战士")  # 卡牌的属性文字标志，作为其他卡牌效果的目标判断
        self.rule_keyword = ("警戒",)  # 卡牌的规则关键词，表示卡牌有些什么关键词
        self.rule_mark = ("响应",)  # 卡牌的规则效果标志，表示卡牌有些什么效果
        self.card_type = "盟友"  # 卡牌类型，表明本卡牌是英雄、盟友、附属还是事件

        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "警戒.\n响应：在刚铎长枪兵被宣告为防御者后，对攻击的敌军造成1点伤害。"

        # 剧情描述的斜体文字
        self.describe_text = "眼前至少有超过一千人的战力，他们的长枪罗列起来，如同浓密的树林。 ——《双城奇谋》"
    ...
    def card_listening(self):
        ...
        if self.active_health_point > 0 and self.main_game.information and self.main_game.information[
            1] == "盟友卡牌宣告防御后" and self.main_game.information[2] == self and "免疫" not in self.main_game.information[
    ...
    def run_card_order(self):
        ...
        if self.card_order[0] == "响应" and self.card_order[1] == 0:
            ...
            if self.card_order[2]:
                ...
                self.main_game.information = [0, "盟友卡牌将要攻击", self, self.active_condition["宣告防御"][0]]
                self.card_order[1] += 1
            ...
            if self.card_order[2]:
                ...
                self.active_condition["宣告防御"][0].active_health_point -= 1
                if self.active_condition["宣告防御"][0].active_health_point < 0:
                    self.active_condition["宣告防御"][0].active_health_point = 0
                self.active_condition["宣告防御"][0].update_mask()
                self.main_game.information = [0, "盟友卡牌攻击后", self, self.active_condition["宣告防御"][0]]
                self.card_order[1] += 1
