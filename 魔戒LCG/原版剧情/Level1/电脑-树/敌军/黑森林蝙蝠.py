...
class Enemy(Enemy_Group):
    def __init__(self, main_game):
        ...
        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "展示后：每位玩家必须选择一名当前被指派执行任务的角色，并将该角色移出本次任务。(所选角色不重置。)"

        # 卡牌有暗影效果的话，代表其暗影效果的文字说明
        self.shadow_text = ""

        # 剧情描述的斜体文字
        self.describe_text = "还有那些巨大的蝙蝠，漆黑得如同黑礼帽一样四下飘荡... ——《哈比人》"
            ...
            if self.card_order[2]:
                ...
                for card in self.main_game.role_area.card_group:
                    if "任务中" in card.active_condition:
                        roles.append(card)
                if roles:
                    card = self.main_game.card_select(roles)
