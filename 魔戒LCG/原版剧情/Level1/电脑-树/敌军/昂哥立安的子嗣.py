...
class Enemy(Enemy_Group):
    def __init__(self, main_game):
        ...
        self.card_amount = (0, 0, 1)  # 这张卡牌在简单、普通和噩梦难度下分别放多少张
        ...
        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "展示后：当前被指派执行任务的每名角色-2意志力并且如果其意志力为0时被弃除，直到本阶段结束。"

        # 卡牌有暗影效果的话，代表其暗影效果的文字说明
        self.shadow_text = "暗影：防御玩家的威胁等级上升5点。(如果本次攻击无人防御，则上升10点代替)"
        ...
        if self.card_order[0] == "展示后" and self.card_order[1] == 0:
            ...
            if self.card_order[2]:
                ...
                for card in self.main_game.role_area.card_group:
                    if "任务中" in card.active_condition:
                        card.active_willpower -= 2
                        if card.active_willpower < -1:
                            card.active_willpower = 0
                            self.role_group.append((card, None))
                        elif card.active_willpower < 0:
                            card.active_willpower = 0
                            self.role_group.append((card, False))
                        else:
                            self.role_group.append((card, True))
                        card.update_mask()
                        if not card.active_willpower:
            ...
            if self.card_order[2]:
                ...
                if self.active_condition["暗影牌"][0].active_condition["攻击中"]:
                    ...
                    self.main_game.threat_area.threat_value += 10
