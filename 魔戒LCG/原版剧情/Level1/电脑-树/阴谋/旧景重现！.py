...
class Enemy(Enemy_Group):
    def __init__(self, main_game):
        ...
        # 规则文字，本卡牌在场时的特殊能力
        self.rule_text = "涌现.\n展示后：将遭遇弃牌堆中最顶部的一张敌军牌或地区牌返回场景区。"

        # 卡牌有暗影效果的话，代表其暗影效果的文字说明
        self.shadow_text = "暗影：为本次攻击分配并结算遭遇弃牌堆中最顶部的一个暗影效果。"
    ...
    def run_card_order(self):
        if self.card_order[0] == "展示后" and self.card_order[1] == 0:
                ...
                if self.main_game.encounterdiscard_area.encounterdiscard_deck:
                    for card in self.main_game.encounterdiscard_area.encounterdiscard_deck:
                        if card.card_type == "敌军" or card.card_type == "地区":
                            break
                if card is not None and (card.card_type == "敌军" or card.card_type == "地区"):
                ...
                if self.main_game.encounterdiscard_area.encounterdiscard_deck:
                    for card in self.main_game.encounterdiscard_area.encounterdiscard_deck:
                        if card.card_type == "敌军" or card.card_type == "地区":
                            break
                if card is not None and (card.card_type == "敌军" or card.card_type == "地区"):
                    self.main_game.encounterdiscard_area.encounterdiscard_deck.remove(card)
                    card.reset_card()
                    card.update_mask()
            ...
            if self.card_order[2]:
                ...
                if self.main_game.encounterdiscard_area.encounterdiscard_deck:
                    for card in self.main_game.encounterdiscard_area.encounterdiscard_deck:
                        if "暗影" in card.rule_mark:
                            break
                if card is not None and "暗影" in card.rule_mark:
            ...
            if self.card_order[2]:
                ...
                if card is not None and "暗影" in card.rule_mark:
                    self.main_game.encounterdiscard_area.encounterdiscard_deck.remove(card)
                    card.reset_card()
                    card.active_condition["暗影牌"] = [self.active_condition["暗影牌"][0]]
                    card.update_mask()
                    if "分配暗影" in self.active_condition["暗影牌"][0].active_condition:
                        self.active_condition["暗影牌"][0].active_condition["分配暗影"].append(card)
                    else:
                        self.active_condition["暗影牌"][0].active_condition["分配暗影"] = [card]
                    ...
                    if self.main_game.card_estimate(self.active_condition["暗影牌"][0])[self.active_condition["暗影牌"][0]]:
                        self.main_game.card_estimate(self.active_condition["暗影牌"][0])[
                            self.active_condition["暗影牌"][0]].append(card)
                    else:
                        self.main_game.card_estimate(self.active_condition["暗影牌"][0])[
                            self.active_condition["暗影牌"][0]] = [card]
                    self.main_game.card_exhibition(card, self.main_game.settings.card_exhibition_time)
                    card.card_order = ["结算暗影", 0, 0, False, 0, 0]
