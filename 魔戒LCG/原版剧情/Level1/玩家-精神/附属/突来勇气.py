...
class Player(Player_Group):
    def __init__(self, main_game):
        ...
        self.card_file = "突来勇气.jpg"  # 卡牌的图像文件名
        self.card_image = pygame.image.load(os.path.join(self.card_dir, self.card_file)).convert()  # 卡牌的原始图像
        self.card_name = "突来勇气"  # 卡牌的名称
        ...
        # 剧情描述的斜体文字
        self.describe_text = "“我在这儿，顽皮小苍蝇；你呀实在胖又懒。抓不到我呀，别想赢，让你在蛛网里气得发疯。” ——比尔博·巴金斯，《哈比人》"
    ...
    def run_card_order(self):
        ...
        if self.card_order[0] == "打出附属后" and self.card_order[1] == 0:
            ...
            if self.card_order[2]:
                ...
                if "附属到" in self.active_condition and "已横置" in self.active_condition["附属到"][0].active_condition:
                    self.main_game.information = [0, self.active_condition["附属到"][0].card_type + "卡牌将要重置", self,
                                                  self.active_condition["附属到"][0]]
                else:
                    self.card_order[2] = 1
                self.card_order[1] += 1
            ...
            if self.card_order[2]:
                ...
                if "附属到" in self.active_condition and "已横置" in self.active_condition["附属到"][0].active_condition:
                    self.active_condition["附属到"][0].active_condition.pop("已横置")
                    self.active_condition["附属到"][0].update_mask()
                    self.main_game.information = [0, self.active_condition["附属到"][0].card_type + "卡牌重置后", self,
                                                  self.active_condition["附属到"][0]]
                self.card_order = [None, 0, 0, None, 0, 0]
