...
class Enemy(Enemy_Group):
    def __init__(self, main_game):
        ...
        self.card_file = "洞穴守卫.jpg"  # 卡牌的图像文件名
        self.card_image = pygame.image.load(os.path.join(self.card_dir, self.card_file)).convert()  # 卡牌的原始图像
        self.card_amount = (2, 2, 2)  # 这张卡牌在简单、普通和噩梦难度下分别放多少张
        self.card_name = "洞穴守卫"  # 这张卡的名称
        self.clash_value = 8  # 这张卡的交锋值
        ...
        self.card_attribute = ("亡灵",)  # 这张卡片的属性文字标志，作为其他卡牌效果的目标判断
        self.rule_keyword = ("厄运",)  # 这张卡片的规则关键词，表示卡片有些什么关键词
        ...
        self.shadow_text = "暗影：选择并弃除一张你控制的附属牌。弃除的目标牌返回场景区。（如果本次攻击无人防御，则以弃除你控制的所有附属牌代替。）"
    ...
    def card_listening(self):
        ...
        if self.active_health_point > 0 and "厄运" in self.rule_keyword and self.main_game.information and \
                self.main_game.information[1][-5:] == "卡牌展示后" and self.main_game.information[3] == self and \
                str(self) not in self.main_game.information:
    ...
    def run_card_order(self):
        ...
        if self.card_order[0] == "展示后" and self.card_order[1] == 0:
            ...
            if self.card_order[2]:
                ...
                affiliateds = self.main_game.find_affiliated()
                if affiliateds:
